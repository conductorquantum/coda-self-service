"""Node provisioning, credential persistence, and OpenVPN management.

This module implements the full node and reconnect lifecycle:

1. **First run** -- the operator provides a one-time ``CODA_NODE_TOKEN``.
...
from coda_node.server.config import PERSISTED_CONFIG_PATH, PERSISTED_PRIVATE_KEY_PATH
...
OPENVPN_PID_PATH = _RUNTIME_DIR / "coda-node-openvpn.pid"
OPENVPN_LOG_PATH = _RUNTIME_DIR / "coda-node-openvpn.log"
...
def _persist_runtime_config(settings: Settings) -> None:
    ...
    persisted = {
        ...
        "node_auto_vpn": settings.node_auto_vpn,
        "node_vpn_profile_path": settings.node_vpn_profile_path,
        "node_machine_fingerprint": settings.node_machine_fingerprint,
    }
...
def _start_openvpn(profile_path: str) -> None:
    ...
    if openvpn_bin is None:
        raise NodeError(
            "openvpn binary not found. Install OpenVPN or disable node auto VPN."
        )
...
async def _wait_for_tunnel(
    ...
    from coda_node.vpn.guard import detect_tun_interface
...
def _as_str(data: dict[str, Any], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise NodeError(f"Node response missing valid '{key}'")
    return value
...
def _as_int(data: dict[str, Any], key: str, default: int) -> int:
    ...
    if isinstance(value, int):
        return value
    raise NodeError(f"Node response has invalid '{key}'")
...
def _resolve_machine_fingerprint(settings: Settings) -> str:
    ...
    fingerprint = settings.node_machine_fingerprint or _machine_fingerprint()
    settings.node_machine_fingerprint = fingerprint
    return fingerprint
...
async def _post_connect(
    ...
    *,
    ...
    Raises:
        NodeError: On non-retryable client errors or after all
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if exc.response.status_code < 500:
                raise NodeError(
                    f"Node request failed ({exc.response.status_code}): "
    ...
    raise NodeError(
        f"Node connect failed after {max_retries} attempts: {last_exc}"
    ) from last_exc
...
async def fetch_node_bundle(settings: Settings) -> dict[str, Any]:
    ...
    Raises:
        NodeError: If the token is empty or the HTTP request fails.
    """
    ...
    return await _post_connect(
        ...
        payload=payload,
        max_retries=settings.node_connect_retries,
    )
...
async def apply_node_bundle(settings: Settings, bundle: dict[str, Any]) -> None:
    """Apply a node bundle to the runtime settings.

    Mutates *settings* in place with QPU identity, JWT credentials,
    Redis URL, API paths, and VPN configuration from the bundle.  If
    ``node_auto_vpn`` is enabled and the bundle includes a VPN
    ...
    Args:
        ...
        bundle: Raw JSON bundle from :func:`fetch_node_bundle`
            or :func:`fetch_reconnect_bundle`.

    Raises:
        NodeError: If required fields are missing or the VPN
            setup fails.
    """
    ...
    if isinstance(jwt_private_key, str) and jwt_private_key:
        ...
        raise NodeError("Node response missing valid 'jwt_private_key'")
    ...
    if isinstance(jwt_key_id, str) and jwt_key_id:
        ...
        raise NodeError("Node response missing valid 'jwt_key_id'")
    ...
    if isinstance(vpn, dict):
        ...
        if settings.vpn_required and (not isinstance(profile, str) or not profile):
            raise NodeError(
                "VPN is required but node response did not include a VPN profile. "
                "Check VPN infrastructure status in the coda admin panel."
            )

        if settings.node_auto_vpn and isinstance(profile, str) and profile:
            await asyncio.to_thread(
                _write_vpn_profile, settings.node_vpn_profile_path, profile
            )
            from coda_node.vpn.guard import detect_tun_interface
...
async def connect_settings(settings: Settings) -> None:
    ...
    On first run (token present), fetches a node bundle.  On
...
async def ensure_persisted_vpn(settings: Settings) -> None:
    ...
    Raises:
        NodeError: If VPN is required but the persisted profile
            is missing, or if the tunnel fails to come up.
    """
    if not PERSISTED_CONFIG_PATH.exists():
        return

    if not settings.node_auto_vpn:
        return

    profile_path = Path(settings.node_vpn_profile_path)
    if not profile_path.exists():
        if settings.vpn_required:
            raise NodeError(
                "Persisted VPN profile not found. Re-run node provisioning with a new token."
            )
        return
